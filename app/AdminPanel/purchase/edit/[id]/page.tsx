...
export default function EditPurchasePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch(`/api/purchase/${id}`).then(r => {
        if (!r.ok) throw new Error("Purchase not found");
        return r.json();
      })
    ]).then(([sData, pData, pur]) => {
      setSuppliers(sData.suppliers || []);
      setProducts(pData.products || []);
      setSupplierId(String(pur.supplierId));
      setInvoiceNumber(pur.invoiceNumber || "");
      setInvoiceDate(pur.invoiceDate ? new Date(pur.invoiceDate).toISOString().split("T")[0] : "");
      setPurchaseNumber(pur.purchaseNumber);
      
      const mappedRows = pur.items.map((item: any) => ({
  ...
  const handleSave = async () => {
    ...
    // Validations:
    // Supplier required (checked above)
    // At least one product required with qty > 0
    const valid = rows.filter(r => r.productId && r.batchNumber && r.qty && r.purchaseRate);
    if (valid.length === 0) {
      setError("Please add at least one product row with batch number, quantity, and purchase rate");
      return;
    }
    ...
    try {
      ...
      setTimeout(() => router.push("/AdminPanel/purchase/list?success=Purchase updated successfully"), 1500);
    } catch (err: any) {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-24">
        ...
        <p className="text-slate-500 font-medium">Loading purchase record...</p>
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-black text-slate-800">Edit Purchase</h1>
          <p className="text-slate-500 mt-0.5">Update purchase entry: #{purchaseNumber}</p>
        </div>
        <Link href="/AdminPanel/purchase/list" className="btn-secondary text-sm">View All Purchases</Link>
      </div>
      ...
      <div className="page-card p-6 space-y-6">
        ...
        <div className="flex gap-3 pt-2">
          <Link href="/AdminPanel/purchase/list" className="btn-secondary flex items-center justify-center">Cancel</Link>
