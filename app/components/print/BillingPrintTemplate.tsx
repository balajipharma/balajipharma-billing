...
export default function BillingPrintTemplate({ invoice, company }: { invoice: InvoiceData; company: CompanySettings }) {
    ...
    <div id="invoice-print" className="invoice-page bg-white text-black">
      ...
      <div style={{ display: "flex", justifyContent: "space-between", borderBottom: "2px solid black", paddingBottom: "6px", marginBottom: "6px" }}>
        ...
        <div style={{ textAlign: "center", borderLeft: "1px solid black", borderRight: "1px solid black", padding: "0 10px", minWidth: "150px" }}>
          <div style={{ fontSize: "11.5px", fontWeight: "bold" }}>GST Tax Invoice</div>
          <div style={{ fontSize: "10px", color: "#555" }}>Original</div>
          <div style={{ fontSize: "15px", fontWeight: "900", margin: "3px 0" }}>TAX INVOICE</div>
          <div style={{ fontSize: "11.5px" }}>Bill No. <strong>{invoice.invoiceNumber}</strong></div>
          <div style={{ fontSize: "11.5px" }}>Bill Dt. <strong>{new Date(invoice.invoiceDate).toLocaleDateString("en-IN")}</strong></div>
        </div>
        {/* Customer Info */}
        <div style={{ flex: 1, paddingLeft: "12px", minWidth: "0" }}>
          <div style={{ fontSize: "11px", color: "#555" }}>M/s.</div>
          <div style={{ fontSize: "13.5px", fontWeight: "bold", lineHeight: "1.1" }}>{invoice.customer.storeName}</div>
          <div style={{ fontSize: "11.5px", whiteSpace: "pre-wrap", marginTop: "3px" }}>{invoice.customer.address}</div>
          {invoice.customer.dlNumber && <div style={{ fontSize: "11.5px", marginTop: "3px" }}>DL No. {invoice.customer.dlNumber}</div>}
          {invoice.customer.gstNumber && <div style={{ fontSize: "11.5px" }}>GST No. {invoice.customer.gstNumber}</div>}
        </div>
      </div>
      ...
      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "11.5px", borderTop: "1px solid black" }}>
        <tbody>
          <tr>
            ...
            <td style={{ ...tdStyle, fontWeight: "bold", border: "1px solid #777" }}>{Number(invoice.subTotal).toFixed(2)}</td>
          </tr>
        </tbody>
      </table>
      ...
      <div style={{ display: "flex", gap: "10px", marginTop: "6px" }}>
        ...
        <div style={{ flex: 1, minWidth: "0" }}>
          ...
          <div style={{ border: "1px solid black", marginTop: "6px", padding: "4px", fontSize: "11.5px", lineHeight: "1.2" }}>
          ...
          <div style={{ marginTop: "3px", fontSize: "10px", color: "#555" }}>E. &amp; O.E. &nbsp; Subject to Rajkot Jurisdiction</div>
        ...
        <div style={{ width: "200px", flexShrink: 0 }}>
          ...
          <div style={{ marginTop: "20px", textAlign: "right", fontSize: "11.5px" }}>
            ...
            <div style={{ marginTop: "35px", borderTop: "1px solid black", paddingTop: "3px", textAlign: "center" }}>Authorized Signatory</div>
          </div>
        </div>
      </div>
    </div>
